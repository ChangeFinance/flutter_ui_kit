...
void main() {
  group('TimeFrameSelector', () {
    testWidgets('renders time Frame selector', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(onChange: onChangeTextField)));
      expect(find.byType(TimeFrameSelector), findsOneWidget);
    });
    testWidgets('click on button', (WidgetTester tester) async {
      void onChangeTextField(String value) {}
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(onChange: onChangeTextField)));
      expect(find.byType(PlainButton).evaluate().length, 5);
    ...
    testWidgets('can access period selected from external object', (WidgetTester tester) async {
      ...
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(key: timeFrameSelectorKey, onChange: onChangeTextField)));
      expect(find.byType(TimeFrameSelector), findsOneWidget);
      expect(find.byType(PlainButton).evaluate().length, 5);
      expect(find.byType(FilledButton).evaluate().length, 1);
      await tester.tap(find.text('1Y'));
    ...
    testWidgets('can show 5 years option', (WidgetTester tester) async {
      void onChangeTextField(String value) {}
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(maxTimeFrame: TimeFrame.FIVE_YEARS, onChange: onChangeTextField)));
      expect(find.byType(TimeFrameSelector), findsOneWidget);
      expect(find.byType(PlainButton).evaluate().length, 5);
    ...
    testWidgets('can disable 5 years option', (WidgetTester tester) async {
      ...
      void onChangeTextField(String value) {}
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(key: timeFrameSelectorKey, maxTimeFrame: TimeFrame.ONE_YEAR, onChange: onChangeTextField)));
      expect(find.byType(TimeFrameSelector), findsOneWidget);
      expect(find.text('1Y'), findsOneWidget);
      expect(find.text('5Y'), findsOneWidget);
    ...
    testWidgets('can disable 1 year and 5 years options', (WidgetTester tester) async {
      final timeFrameSelectorKey = new GlobalKey<TimeFrameSelectorWidgetState>();

      void onChangeTextField(String value) {}
      await tester.pumpWidget(wrapInMaterialApp(TimeFrameSelector(key: timeFrameSelectorKey, maxTimeFrame: TimeFrame.ONE_MONTH, onChange: onChangeTextField)));
      ...
      expect(timeFrameSelectorKey.currentState.selectedTimeFrame, TimeFrame.ONE_WEEK);

      await tester.tap(find.text('1M'));
      await tester.pump();

      expect(timeFrameSelectorKey.currentState.selectedTimeFrame, TimeFrame.ONE_MONTH);
